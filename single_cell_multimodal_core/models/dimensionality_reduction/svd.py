...
class TruncatedSVDMixin(DimensionalityReductionMixinABC):
    ...
    def fit_and_apply_dimensionality_reduction(self, *, input, output_dimensionality=64) -> np.array:
        ...
        logger.info(
            f"fitting and applying svd to an input of dimension {input.shape[1]}, with target dimensionality of {output_dimensionality}"
        )
        output = self._svd.fit_transform(input)
        logger.info(
            f"fitting and applying svd to an input of dimension {input.shape[1]}, with target dimensionality of {output_dimensionality} - Done"
        )

        logger.debug(
            f"Reduced X shape to {output_dimensionality} dimension with SVD, the memory usage is now:"
    ...
    def apply_dimensionality_reduction(self, *, input) -> np.array:
        ...
        logger.debug(
            f"Reduced X shape to {self._svd.n_components} dimension with SVD, the memory usage is now:"
            f"  {str(input.shape):14} {input.size*4/1024/1024/1024:2.3f} GByte"
        )
