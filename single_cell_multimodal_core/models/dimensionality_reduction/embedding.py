...
class EmbeddingDimensionalityReductionMixinABC(DimensionalityReductionMixinABC, metaclass=abc.ABCMeta):
    ...
    def fit_and_apply_dimensionality_reduction(
        self, *, input, output_dimensionality=64
    ) -> np.array:  # todo clean method argument
        logger.info(
            f"Istantiating the embedder {self.embedder_class} with input_dim={input.shape[1]} and latent_dim={output_dimensionality}"
        )

        if self._embedder is None:
            self._embedder = self.embedder_class(
                input_dim=input.shape[1], latent_dim=output_dimensionality, **self.embedder_kwargs
            )
...
class AutoEncoderDimensionalityReductionMixin(AutoEncoderMixin, EmbeddingDimensionalityReductionMixinABC):
    ...
