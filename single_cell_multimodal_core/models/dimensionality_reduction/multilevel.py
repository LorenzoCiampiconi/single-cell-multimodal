...
class MultiLevelDimensionalityReductionWrapperMixin(DimensionalityReductionMixinABC):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.intermediate_dims: List[int] = self.configuration['dimensionality_reduction_params']['intermediate_dimensionalities']
        self._dimensionality_reducers_class: List[Type[DimensionalityReductionMixinABC]] = [self._make_reductor_type_instatiable(reductor_type) for reductor_type in self.configuration['dimensionality_reduction_params']['dimensionality_reducers_class']]
    ...
    def _make_reductor_type_instatiable(self, original_type:Type[DimensionalityReductionMixinABC]) -> Type[DimensionalityReductionMixinABC]:
        """
    ...
    def _instantiate_reducers(self, force=False):
        if self.dimensionality_reducers is None or force:
            ...
            for dimensionality_reducer_class in self._dimensionality_reducers_class:
                dimensionality_reducers.append(dimensionality_reducer_class(seed=self.seed)) #todo, this is not instatiating the seed, develop it
    ...
    def fit_and_apply_dimensionality_reduction(self, *, input, output_dimensionality=64) -> np.array:
        ...
        for i, reducer in enumerate(self.dimensionality_reducers[:-1]):
            ...
            logger.info(f"Applying dimensionality reduction step {i}, from {input.shape[1]} to {latent_dim}, with {reducer}")
            input = reducer.fit_and_apply_dimensionality_reduction(input=input, output_dimensionality=latent_dim)

        return self.dimensionality_reducers[-1].fit_and_apply_dimensionality_reduction(input=input, output_dimensionality=output_dimensionality)
